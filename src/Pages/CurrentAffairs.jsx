  ...
  useEffect(() => {
    const fetchCurrentPosts = async () => {
      try {
        ...
        const filtered = data.filter((post) => post.category === "Current Affairs");
        setCurrentPosts(filtered);
      } catch (err) {
        console.error("Error fetching Current Affairs posts:", err);
        setError("Failed to load Current Affairs articles.");
      } finally {
        setLoading(false);
      }
    };
      ...
      >
        ...
        <div className="relative z-10 max-w-3xl mx-auto text-center px-6">
          ...
          <p className="text-lg sm:text-xl mb-6 opacity-90">
            All articles in our Current Affairs category.
          </p>
        </div>
